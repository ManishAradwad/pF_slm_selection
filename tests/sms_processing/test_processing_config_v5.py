...
from pocketfinancer_sms.configuration_v5 import (
    NATIVE_INTEGRATION_RELEASE_V5,
    ProcessingConfigSnapshotV5,
...
def _snapshot(
    *,
    ...
    return ProcessingConfigSnapshotV5(
        ...
        release_id=NATIVE_INTEGRATION_RELEASE_V5,
...
def _schema() -> dict:
    return json.loads(
        (ROOT / "configs/sms_processing/contracts/v5/processing-config.schema.json").read_text()
    )
...
def test_successor_configuration_binds_automatic_policy() -> None:
    ...
    assert payload["contract"] == "pocketfinancer.processing-config/5"
    assert payload["contract_release"]["release_id"] == "native-integration-v5"
...
def test_stored_operations_retain_release_owned_routing() -> None:
    ...
    assert (
        rollout_mode_for_stored_release(
            "native-integration-v5", RolloutMode.AUTOMATIC
        )
        == RolloutMode.AUTOMATIC
    )
    with pytest.raises(ValueError, match="incompatible rollout"):
        rollout_mode_for_stored_release(
            "native-integration-v5", RolloutMode.REVIEW_ONLY
        )
