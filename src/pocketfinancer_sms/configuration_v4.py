"""Immutable, canonically hashed processing configuration with model identity provenance."""
...
PROCESSING_CONFIG_CONTRACT_V4 = "pocketfinancer.processing-config/4"
MODEL_IDENTITY_FILE_SHA256 = "file_sha256"
MODEL_IDENTITY_SYSTEM_MANAGED_RUNTIME = "system_managed_runtime"
_MODEL_IDENTITY_KINDS = {MODEL_IDENTITY_FILE_SHA256, MODEL_IDENTITY_SYSTEM_MANAGED_RUNTIME}


@dataclass(frozen=True, slots=True)
class ExtractorRuntimeConfigV4:
    eligible: bool
    ...
    def __post_init__(self) -> None:
        if self.model_identity_kind not in _MODEL_IDENTITY_KINDS:
            raise ValueError("extractor model identity kind is unsupported")
        if self.eligible:
        ...
        if (
            self.model_identity_kind == MODEL_IDENTITY_FILE_SHA256
            and self.model_file_sha256 is not None
            and not _is_sha256(self.model_file_sha256)
        ):
            raise ValueError("extractor model hash is invalid")
        if (
            self.eligible
            and self.model_identity_kind == MODEL_IDENTITY_FILE_SHA256
            and self.model_file_sha256 is None
        ):
            raise ValueError("eligible file-backed extractor requires a model hash")
        if (
            self.model_identity_kind == MODEL_IDENTITY_SYSTEM_MANAGED_RUNTIME
            and self.model_file_sha256 is not None
        ):
            raise ValueError("system-managed extractor must not claim a model file hash")
...
@dataclass(frozen=True, slots=True)
class ProcessingConfigSnapshotV4:
    operation_id: str
    ...
    contract: str = PROCESSING_CONFIG_CONTRACT_V4
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.release_id != "native-integration-v4"
        ...
        if self.contract != PROCESSING_CONFIG_CONTRACT_V4:
