#!/usr/bin/env python3
"""Export or verify the exact frozen native-integration-v6 app asset bundle."""
...
MANIFEST_SHA256 = "b7ce1047c998dceae8935486a9982c0bda663ff43cebaaa284f6d542a81ab3e8"
RELEASE_ID = "native-integration-v6"
...
def load_frozen_assets(repo_root: Path = REPO_ROOT) -> tuple[FrozenAsset, ...]:
    ...
    if _sha256(manifest_content) != MANIFEST_SHA256:
        raise BundleIntegrityError("native-integration-v6 manifest does not match its frozen hash")
    try:
        manifest = json.loads(manifest_content.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise BundleIntegrityError("native-integration-v6 manifest is not valid UTF-8 JSON") from exc
    if not isinstance(manifest, dict) or manifest.get("release_id") != RELEASE_ID:
        raise BundleIntegrityError("native-integration-v6 manifest has the wrong release ID")
    artifacts = manifest.get("artifacts")
    if not isinstance(artifacts, list) or not artifacts:
        raise BundleIntegrityError("native-integration-v6 manifest has no artifact table")

    frozen = [FrozenAsset(MANIFEST_PATH, MANIFEST_SHA256, manifest_content)]
    seen = {MANIFEST_PATH}
    for item in artifacts:
        if not isinstance(item, dict):
            raise BundleIntegrityError("native-integration-v6 contains an invalid artifact entry")
...
def check_bundle(bundle_root: Path, repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    if failures:
        raise BundleIntegrityError("packaged v6 bundle failed verification: " + ", ".join(failures))
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Export or verify the pinned native-integration-v6 app asset bundle."
    )
