#!/usr/bin/env python3
"""Build native-integration-v3 after every extractor freeze check passes."""
...
ARTIFACTS = LEGACY_ARTIFACTS + NEW_ARTIFACTS
...
_SCHEMA_PATHS = tuple(path for path, _ in NEW_ARTIFACTS if path.endswith(".schema.json"))
...
def validate_freeze_inputs() -> None:
    ...
    if missing:
        raise ValueError(f"native-integration-v3 assets are incomplete: {', '.join(missing)}")
    if len({path for path, _ in ARTIFACTS}) != len(ARTIFACTS):
        raise ValueError("native-integration-v3 contains duplicate artifact paths")
...
def build_manifest() -> dict[str, Any]:
    ...
    jsonschema.validate(
        manifest,
        _read_json("configs/sms_processing/contracts/releases/v3/release-manifest.schema.json"),
    )
    return manifest
